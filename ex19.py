...
def getInput():
    ...
    while True:
        ...
        result.append(line)
    toReturn = list(result)
    print('Input: ' + str(toReturn))
    return toReturn

def sortPeople():
    userInput = getInput()
    # first sort by score
    userInput.sort(key=lambda tup: tup[2])
    # then sort by age
    userInput.sort(key=lambda tup: tup[1])
    # then sort by score
    userInput.sort(key=lambda tup: tup[0])
    print('Sorted: ' + str(userInput))
sortPeople()
